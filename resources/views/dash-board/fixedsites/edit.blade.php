...
<body class="g-sidenav-show rtl bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg overflow-x-hidden">
        ...
        <div class="row backgroundW p-4 m-3">
            ...
            <form action="{{ route('fixedsites.update' , $fixedSites->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <h4>الدولة:</h4>
                <select style="width: 200px" class="form-cdontrol" id="countries" name="countries">
                    ...
                    @foreach ($countries as $get_country)
                        <option value="{{$get_country->id}},{{$get_country->show_status}}" {{$fixedSites->country_id == $get_country->id  ? 'selected' : ''}}>{{$get_country->country_name}}</option>
                    @endforeach
                </select>
                <div class="col-md-12 mb-3">
                    <label for="Description" class="form-labell ">اسم الموقع</label>
                    <input type="text" name="site_name" value="{{$fixedSites->site_name}}" class="form-controll">
                </div>

                <div class="col-md-12 mb-3">
                    <label for="Description" class="form-labell ">رابط الموقع</label>
                    <input type="text" name="href" value="{{$fixedSites->href}}" class="form-controll">
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <img src="{{ url('../public/uploading/' . $fixedSites->photo) }}" alt="" class="img-tumbnail" width="70" height="70" style="border-radius: 100%">
                </div>

                <div class="col-12">
                    <button type="submit" class="btn btn-dark" style="background-color:#42424a">تحديث</button>
                </div>
            </form>
        </div>
